...
@section('page-description', 'Perbarui informasi jadwal perkuliahan')
    ...
    <div class="form-card p-8">
        ...
        <div class="text-center mb-8">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style="background: linear-gradient(135deg, #B8CFCE, #7F8CAA);">
                <i data-lucide="edit" class="w-8 h-8" style="color: #333446;"></i>
            </div>
            <h2 class="text-3xl font-bold mb-2" style="color: #EAEFEF;">Edit Jadwal</h2>
            <p style="color: #7F8CAA;">Perbarui informasi jadwal perkuliahan</p>
        </div>
        ...
        <form action="{{ route('jadwal.update', $jadwal->id) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <select name="kelas_id" class="form-input" required>
                    <option value="" style="background: #333446;">-- Pilih Kelas --</option>
                    @foreach($kelas as $k)
                    <option value="{{ $k->id }}" {{ $jadwal->kelas_id == $k->id ? 'selected' : '' }} style="background: #333446;">
                        {{ $k->kode_kelas }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="hari" value="{{ $jadwal->hari }}" class="form-input" required>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="time" name="jam_mulai" value="{{ $jadwal->jam_mulai }}" class="form-input" required>
                </div>
                <div class="space-y-2">
                    <label class="block text-sm font-medium mb-2" style="color: #EAEFEF;">
                        <i data-lucide="clock" class="w-4 h-4 inline mr-2" style="color: #B8CFCE;"></i>
                    ...
                    <input type="time" name="jam_selesai" value="{{ $jadwal->jam_selesai }}" class="form-input" required>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="ruangan" value="{{ $jadwal->ruangan }}" class="form-input" required>
            </div>
